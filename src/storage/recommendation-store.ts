...
export async function commitRecommendationSnapshot(
  ...
  const recommendations = [...byRepository.values()]
    .sort((left, right) => (
  ...
  const attemptAt = now();
  return db.transaction('rw', db.recommendations, db.recommendationState, async () => {
    await db.recommendations.clear();
...
/** Query cache rows for one account, excluding repositories that have since joined the library. */
export async function listRecommendations(accountLogin: string): Promise<RecommendationRecord[]> {
  const key = accountKey(accountLogin);
  const [stored, stars] = await Promise.all([
  ...
  return stored
    .filter((recommendation) => !liveLibrary.has(recommendation.repositoryKey))
    .sort((left, right) => (
      right.score - left.score
        || right.stargazerCount - left.stargazerCount
        || left.repositoryKey.localeCompare(right.repositoryKey)
