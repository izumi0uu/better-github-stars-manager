...
const messages: Record<Locale, AgentDiagnosticsMessages> = {
  en: {
    openAgentDiagnostics: 'Open Cubby Diagnostics',
    title: 'Cubby Diagnostics',
    provider: 'Provider',
    ...
    viewsLabel: 'Cubby diagnostics views',
    analysis: 'Analysis',
    ...
    rawArmed: (captureId) => `Armed for the next real Cubby run (${captureId})`,
    rawCapturing: (rootOperationId) => `Capturing ${rootOperationId ?? 'Cubby run'}`,
    rawCompleted: (rootOperationId) => `Capture completed for ${rootOperationId ?? 'Cubby run'}`,
    captureNextRun: 'Capture next run',
    disarm: 'Disarm',
    droppedEvents: (count, bytes, reason) => `${count} event(s), ${bytes} byte(s), ${reason}`,
    operations: 'Operations',
    noOperations: 'No Cubby operation has been recorded in this development build.',
    timeline: 'Timeline',
    ...
    clearTraceConfirmation: 'Clear all local Cubby diagnostic traces?',
  ...
  'zh-CN': {
    openAgentDiagnostics: '打开 Cubby 诊断',
    title: 'Cubby 诊断',
    provider: 'Provider',
    ...
    rawArmed: (captureId) => `已为下一次真实 Cubby 运行启用捕获 (${captureId})`,
    rawCapturing: (rootOperationId) => `正在捕获 ${rootOperationId ?? 'Cubby 运行'}`,
    rawCompleted: (rootOperationId) => `已完成 ${rootOperationId ?? 'Cubby 运行'} 的捕获`,
    captureNextRun: '捕获下一次运行',
    disarm: '停用捕获',
    droppedEvents: (count, bytes, reason) => `${count} 个事件，${bytes} 字节，${reason}`,
    operations: '操作',
    noOperations: '此开发构建尚未记录任何 Cubby 操作。',
    timeline: '时间线',
    eventType: '事件类型',
    allEvents: '所有事件',
    ...
    clearTraceConfirmation: '要清除所有本地 Cubby 诊断跟踪吗？',
