...
describe('Options preferences', () => {
  ...
  it('rejects a classic PAT missing required scopes and surfaces the inline error', async () => {
    authMocks.getConfig.mockResolvedValue(config());
    authMocks.hasToken.mockResolvedValue(false);
    authMocks.setToken.mockRejectedValue(new Error('TOKEN_GIST_MISSING'));

    await renderOptions();

    const input = document.querySelector<HTMLTextAreaElement>('textarea[placeholder="ghp_..."]');
    expect(input).toBeInstanceOf(HTMLTextAreaElement);
    await setTextareaValue(input!, 'github_pat_partial_scopes');
    const save = [...document.querySelectorAll<HTMLButtonElement>('button')]
      .find((button) => button.textContent?.trim() === 'Save & verify');
    await click(save!);

    expect(authMocks.setToken).toHaveBeenCalledWith('github_pat_partial_scopes');
