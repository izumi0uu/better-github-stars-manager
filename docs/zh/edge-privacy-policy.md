...
运行和发布证据经过有界处理和脱敏。它可能包含脱敏的浏览器身份、可执行文件路径摘要、扩展 ID、场景标识、诊断计数、安装包指纹、manifest 权限事实和能力对象。
...
扩展通过 HTTPS 向 GitHub 和内建 Provider 发送请求。自定义 localhost 服务只有在你配置并允许精确的本地地址后才能使用 HTTP。扩展把全部可执行代码都包含在安装包内，不会获取或执行远程脚本。GitHub 和 Provider 响应数据会被当作数据，而不是可执行的扩展代码。
...
- [支持与隐私 issue 追踪](https://github.com/izumi0uu/better-github-stars-manager/issues)
