...
配额充足时，连续的步骤会在同一次唤醒内接着跑，所以大关注列表在几分钟内收敛，而不是横跨多次小时级 alarm。一旦再跑一整步会花到配额储备以下，链式执行立即停止，被中断的扫描因此永远不会把 Stars 同步、Watch 或 Cubby 的配额抢光。只有请求预算暂停才可能链式继续：截止时间暂停说明这次唤醒本来就慢，配额暂停则是必须等到 GitHub 重置时间的硬等待。当剩余配额或该步的请求成本未知时，链式执行同样停止——无法测量的一步可能花到储备以下。
